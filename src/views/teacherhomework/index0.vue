  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="课程" align="center">
        <template slot-scope="scope">
          <!-- {{ scope.row.class.courses }} -->
          <div>
            <ul>
              <li v-for="(item, index) in scope.row.class.courses" :key="index">
                  <router-link :to="{name:'/Teacherhomework1'}">{{ item.name }}</router-link> 
              </li>
            </ul>
          </div>
        </template>
      </el-table-column>
      <el-table-column class-name="status-col" label="作业" align="center">
        <template slot-scope="scope">
...
<script>
import { putstudent,postList, deleteList, postfile,deletestudent } from '@/api/form'
import { classList ,coueseList} from '@/api/menu'
import { getList,getodataList,} from '@/api/tree'
export default {
  ...
  methods: {
      ...
      fetchData() {
        ...
        getList().then(({ data }) => {
          ...
          /* this.list = response.value.filter(item => {
